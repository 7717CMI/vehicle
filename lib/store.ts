...
export const useDashboardStore = create<DashboardStore>((set, get) => ({
  ...
  updateFilters: (newFilters) => {
    ...
    set((state) => {
      // When switching data type, clear segments to avoid stale selections
      if (newFilters.dataType && newFilters.dataType !== state.filters.dataType) {
        console.log(`🔧 Store: Switching data type to ${newFilters.dataType} - clearing segments`)
        newFilters = {
          ...newFilters,
          segments: [],
        }
